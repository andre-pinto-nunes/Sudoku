...
debut:					ori 	$v0, $0, 54
					la 	$a0, msg_debut
					la 	$a1, nom_du_fichier 
					la 	$a2, 20
					syscall
					ori $t1, $0, 0
					lb $a0, nom_du_fichier($t1)
repet:										# le input a un newline a la fin. on recopie toute la string sauf le newline
					sb $a0, nom_du_fichier_corrige($t1)
					addi $t1, $t1,1
					lb $a0, nom_du_fichier($t1)
					bne $a0, 10, repet
...
probleme_grille_depart:			ori	$v0, $0, 55
					la	$a0, msg_erreur
					la	$a1, 0
					syscall
					ori 	$t1, $0, 0
...
saut_de_ligne2:				sb	$a0, chaine_finale($t1)
#syscall
					addi	$t1, $t1, 1
